...
def configure_interface():
    ...
    if uploaded_file is not None:
        ...
        if blob_url:
            st.write(f"Arquivo{fileName} enviado para o Azure Blob Storage")
...
def show_image_and_validation(blob_url, credit_card_info):
    ...
    st.write("Resutado da Validação")
    if credit_card_info and credit_card_info["card_name"]:
        st.markdown(f"<h1 style='color: green;'>Cartão Válido</h1>", unsafe_allow_html=True)
        st.write(f"Nome do Tutular: {credit_card_info['card_name']}")
        st.write(f"Banco Emissor: {credit_card_info['bank_name']}")
        st.write(f"Data de validade: {credit_card_info['expired_date']}")

    else:
        st.markdown(f"<h1 style='color: red;'>Cartão Inválido</h1>", unsafe_allow_html=True)
        st.write(f"Este não é um numero de cart~çao válido.")
